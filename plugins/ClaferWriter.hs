              ...
              case graph of
                Right CompilerResult { extension = ext,
                    ...
                    return $ RawBlock "html" ((if withGraph then out else "") ++ (if withGraph && withStats then "<br>\n" else "") ++ 
                                                (if withStats then "Module Statistics:<br>\n<span class=\"summary\">" ++ unlines (map (++"<br>") (lines stats)) else "") ++
                                                if withLinks then "</class><br>\nModule Downloads: <a href=clafer/" ++ fileName ++ ".cfr>[.cfr]</a> <a href=clafer/" ++ fileName ++ ".html>[.html]</a>" else "")
